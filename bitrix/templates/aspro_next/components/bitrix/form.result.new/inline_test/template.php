...
<div class="form inline <?=$arResult["arForm"]["SID"]?>" id="inline">
	<!--noindex-->
	<div class="form_head">
		<?if($arResult["isFormTitle"] == "Y"):?>
			<div class="form-title-page"><?=$arResult["FORM_TITLE"]?></div>
		<?endif;?>
...
<!--                    --><?//=GetMessage("FORM_SUCCESS");?>
    ...
    <form
            <?=$arResult["arForm"]["STAT_EVENT3"]?>
            class='col formRec'
            name="<?= $arResult['WEB_FORM_NAME'] ?>"
            action="<?=((!empty($_SERVER['HTTPS'])) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];?>"
            onsubmit="submitForm(this, '.form'); return false;"
            method="POST"
            novalidate="novalidate"
            id="<?=$arResult["arForm"]["SID"]?>"
		...
		<div class="form_footer">
			...
			<?if($bShowLicenses == "Y"):?>
				<div class="licence_block filter label_block">
					<input type="checkbox" id="licenses_inline" <?=(COption::GetOptionString("aspro.next", "LICENCE_CHECKED", "N") == "Y" ? "checked" : "");?> name="licenses_inline" required >
...
<!--			<button type="submit" class="button medium" value="submit" name="web_form_submit" ><span>--><?//=$arResult["arForm"]["BUTTON"]?><!--</span></button>-->
			<input type="submit" class="btn btn-default" value="<?=$arResult["arForm"]["BUTTON"]?>" name="web_form_submit" id="web_form_submit">
