...
<?if(true):?>
    ...
    <?if($isAjax=="N"){
        ...
        $frame->begin();?>
    <?}?>
    <?php $GLOBALS['SHOWALL_1']=true;//чтобы в таблице выводились все элементы раздела?>
			...
			<?if($isAjax === 'N'){?>
                ...
                <?$APPLICATION->IncludeComponent(
                    ...
                    Array(
                        ...
                        "PAGER_DESC_NUMBERING" => $arParams["PAGER_DESC_NUMBERING"],
                        "PAGER_DESC_NUMBERING_CACHE_TIME" => $arParams["PAGER_DESC_NUMBERING_CACHE_TIME"],
                        "PAGER_SHOW_ALL" => "Y",//$arParams["PAGER_SHOW_ALL"],
                        "AJAX_OPTION_ADDITIONAL" => "",
                        "ADD_CHAIN_ITEM" => "N",
                        "SHOW_QUANTITY" => $arParams["SHOW_QUANTITY"],
                        "SHOW_QUANTITY_COUNT" => $arParams["SHOW_QUANTITY_COUNT"],
                ...
                <?if($USER->isAdmin()):?>
                    <?
                        ...
                        $entitySections = Section::compileEntityByIblock($arParams["IBLOCK_ID"]);
                        ...
                        $getSections = $getSections[0];
                        ?>
                    <?if($getSections['UF_ONE_OBLAST']):?>
                        <?if($getSections['UF_ONE_TITLE']):?>
                            <h2 class="title-cat"><?=$getSections['UF_ONE_TITLE']?></h2>
                        <?endif?>
                        <div class="one-line-desc">
                            <?if($getSections['UF_FIRST_IMAGE']):?>
                                <a class="left-image popup_link fancy" href="<?=CFile::getPath($getSections['UF_IMAGE_ONE_BIG'])?>" data-fancybox-group="item_category" >
                                    <img src="<?=CFile::getPath($getSections['UF_FIRST_IMAGE'])?>" alt="">
                                </a>
                            <?endif;?>
                            <div class="desc-block"><?=$getSections['UF_ONE_OBLAST']?></div>
                        </div>
                    <?endif;?>

                        <?if($getSections['UF_TWO_OBLAST']):?>
                            <?if($getSections['UF_TWO_TITLE']):?>
                                <h3 class="title-cat"><?=$getSections['UF_TWO_TITLE']?></h3>
                            <?endif?>
                            <div class="one-line-desc">
                                <?if($getSections['UF_TWO_IMAGE']):?>
                                    <a class="mobile right-image popup_link fancy" href="<?=CFile::getPath($getSections['UF_IMAGE_TWO_BIG'])?>" data-fancybox-group="item_category">
                                        <img src="<?=CFile::getPath($getSections['UF_TWO_IMAGE'])?>" alt="">
                                    </a>
                                <?endif;?>
                                <div class="desc-block"><?=$getSections['UF_TWO_OBLAST']?></div>
                                <?if($getSections['UF_TWO_IMAGE']):?>
                                    <a class="not-mobile right-image popup_link fancy" href="<?=CFile::getPath($getSections['UF_IMAGE_TWO_BIG'])?>" data-fancybox-group="item_category">
                                        <img src="<?=CFile::getPath($getSections['UF_TWO_IMAGE'])?>" alt="">
                                    </a>
                                <?endif;?>
                            </div>
                        <?endif;?>
                        <?if($getSections['UF_THREE_OBLAST']):?>
                            <?if($getSections['UF_THREE_TITLE']):?>
                                <h3 class="title-cat"><?=$getSections['UF_THREE_TITLE']?></h3>
                            <?endif?>
                            <div class="one-line-desc">
                                <?if($getSections['UF_THREE_IMAGE']):?>
                                    <a class="left-image popup_link fancy" href="<?=CFile::getPath($getSections['UF_THREE_IMAGE_BIG'])?>" data-fancybox-group="item_category">
                                        <img src="<?=CFile::getPath($getSections['UF_THREE_IMAGE'])?>" alt="">
                                    </a>
                                <?endif;?>
                                <div class="desc-block"><?=$getSections['UF_THREE_OBLAST']?></div>
                            </div>
                        <?endif;?>
