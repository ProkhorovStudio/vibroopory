...
//    $landingPage = null;
//    if(\Bitrix\Main\Loader::includeModule('clickon.landingsbyfilter')){
////    if(!$arResult['VARIABLES']['SECTION_ID']){
//        $filterResolver = \ClickON\LandingsByFilter\FilterResolver::getInstance();
//        $landingPage = $filterResolver->detectLandingPage($arResult);
////    }
//    }
...
if($section){
	...
	$posSectionDescr = COption::GetOptionString("aspro.next", "SHOW_SECTION_DESCRIPTION", "TOP", SITE_ID);
...
//поддержку мета тега robots, если установлено доп.поле у секции
include("utils.php");
setMetaRobotsFromSection($arParams["IBLOCK_ID"], $arSection["ID"]);
...
<div class="js_wrapper_items" data-params='<?=str_replace('\'', '"', CUtil::PhpToJSObject($arTransferParams, false))?>'>
    ...
    @include_once('page_blocks/'.$arParams["SECTION_ELEMENTS_TYPE_VIEW"].'-landing_test.php');
