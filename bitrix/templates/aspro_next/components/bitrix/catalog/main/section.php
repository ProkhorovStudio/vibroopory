...
$select = ["UF_BREADCRUMBS", "SECTION_PAGE_URL"];
$sort = ["SORT" => "ASC"];
$filter = [
...
$val = [];
while($arRes = $rsResult->GetNext())
{
    $val[0]["TITLE"] = $arRes['UF_BREADCRUMBS'];
    $val[0]["LINK"] = $arRes['SECTION_PAGE_URL'];
}

if(!empty($val[0]["TITLE"])){
    array_splice($APPLICATION->arAdditionalChain, 0,1, $val);
}
