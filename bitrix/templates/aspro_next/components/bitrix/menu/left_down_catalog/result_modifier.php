...
if($arSections){
	...
	foreach($arSections as $ID => $arSection){
		...
		if($arSection['DEPTH_LEVEL'] == 1 && $arSection['UF_SECTION_GROUP'] == '20'){
			$arResult[$arSection['UF_CATEGORY_SECTION']]['SECTIONS'][] = &$arSections[$arSection['ID']];
            $rsCategory = CUserFieldEnum::GetList(array(), array(
                "ID" => $arSection['UF_CATEGORY_SECTION'],
            ));
            if($category = $rsCategory ->GetNext()){
                $arResult[$arSection['UF_CATEGORY_SECTION']]['NAME'] = $category['VALUE'];
            }
		}
	}
}?>