						...
						<div class="information <?=($arItem["OFFERS"] && $arItem['OFFERS_PROP'] ? 'has_offer_prop' : '');?>  inner_content js_offers__<?=$arItem['ID'];?>">
							<div class="cost prices clearfix">
								<?if( count( $arItem["OFFERS"] ) > 0 ){?>
									...
									<div class="js_price_wrapper price">
										<?if($arCurrentSKU):?>
											...
											{?>
												<?if($arCurrentSKU['ITEM_PRICE_MODE'] == 'Q' && is_array($arCurrentSKU['PRICE_MATRIX']['ROWS']) && count($arCurrentSKU['PRICE_MATRIX']['ROWS']) > 1):?>
								...
								<?}else{?>
									...
									{?>
										<?if($arItem['ITEM_PRICE_MODE'] == 'Q' && is_array($arItem['PRICE_MATRIX']['ROWS']) && count($arItem['PRICE_MATRIX']['ROWS']) > 1):?>
							...
							<?elseif($arItem["OFFERS"]):?>
								...
								<?}else{?>
									...
									if(isset($arCurrentSKU['PRICE_MATRIX']) && $arCurrentSKU['PRICE_MATRIX']) // USE_PRICE_COUNT
									{?>
										<?if($arCurrentSKU['ITEM_PRICE_MODE'] == 'Q' && is_array($arItem['PRICE_MATRIX']['ROWS']) && count($arCurrentSKU['PRICE_MATRIX']['ROWS']) > 1):?>
											<?$arOnlyItemJSParams = array(
