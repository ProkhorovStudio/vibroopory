...
<?endif;?>
		...
		<?foreach($arBlockOrder as $code):?>
			...
			<?elseif($code == 'ask'):?>
				<?if(($arParams["SHOW_ASK_BLOCK"] == "Y") && (intVal($arParams["ASK_FORM_ID"]))):?>
					<div class="wraps hidden_print drag_block_detail <?=$code?>" id="AskTab">
						<hr>
						<h2 class="form-title-product"><?=($arParams["TAB_FAQ_NAME"] ? $arParams["TAB_FAQ_NAME"] : GetMessage('ASK_TAB'))?></h2>
	...
	array(
		...
		"COMPONENT_TEMPLATE" => "inline_test",
	...
	false
			...
			<?elseif($code == 'reviews'):?>
				<?if($arParams["USE_REVIEW"] == "Y"):?>
					<div class="wraps product_reviews_tab hidden_print <?=$arParams['REVIEWS_VIEW']?>" id="ReviewsTab">
						<hr>
						<h2 class="reviews-title"><?=($arParams["TAB_REVIEW_NAME"] ? $arParams["TAB_REVIEW_NAME"] : GetMessage("REVIEW_TAB"))?><span class="count empty"></span></h2>
