									...
									<div class="item-actions flexbox flexbox--row">
										...
										<div class="item-price">
											<div class="cost prices clearfix">
												...
												if(isset($arSKU['PRICE_MATRIX']) && $arSKU['PRICE_MATRIX'] && is_array($arSKU['PRICE_MATRIX']['ROWS']) && count($arSKU['PRICE_MATRIX']['ROWS']) > 1) // USE_PRICE_COUNT
												{?>
										...
										<div class="item-buttons item_<?=$arSKU["ID"]?> buy_block">
											...
											{?>
												<?if($arSKU['ITEM_PRICE_MODE'] == 'Q' && is_array($arSKU['PRICE_MATRIX']['ROWS']) && count($arSKU['PRICE_MATRIX']['ROWS']) > 1):?>
...
<div class="item_main_info type_clothes type_3_el <?=(!$showCustomOffer ? "noffer" : "");?> <?=($arParams["SHOW_UNABLE_SKU_PROPS"] != "N" ? "show_un_props" : "unshow_un_props");?>" id="<?=$arItemIDs["strMainID"];?>">
	<div class="img_wrapper swipeignore">
		...
		<?$countThumb = is_array($arResult["MORE_PHOTO"]) ? count($arResult["MORE_PHOTO"]) : 0;?>
	...
	<div class="right_info">
		<div class="info_item">
			...
			<?$frame = $this->createFrame()->begin();?>
								...
								<div class="view_sale_block <?=($arQuantityData["HTML"] ? '' : 'wq');?>">
									...
									<?if($arQuantityData["HTML"]):?>
										<div class="quantity_block">
											...
											<div class="values">
												<span class="item">
													<span class="value" <?=((is_array($arResult["OFFERS"]) && count( $arResult["OFFERS"] ) > 0 && $arParams["TYPE_SKU"] == 'TYPE_1' && $arResult["OFFERS_PROP"]) ? 'style="opacity:0;"' : '')?>><?=$totalCount;?></span>
				...
				<?endif;?>
				<?if($arResult['VISIBLE_PROPS']):?>
						<?$iCountProps = is_array($arResult['VISIBLE_PROPS']) ? count($arResult['VISIBLE_PROPS']) : 0;?>
						</div>
						<div class="col-md-6">
							...
							<div class="top_props">
								...
								<div class="props props_list">
									...
									<?endif;?>
										...
										<?endforeach;?>
									<?if($bShowMoreLink):?>
										<div class=""><a class="choise colored more_characteristic" href="#CharacteristicsTab"><?=GetMessage('ALL_CHARS');?></a></div>
                                        <script>
                                            BX.ready(function () {
                                                $('.more_characteristic').on('click', function () {
                                                    $('[tab_id="#CharacteristicsTab"]').click();
	...
	<?if($arResult['OFFERS']):?>
		<span itemprop="offers" itemscope itemtype="http://schema.org/AggregateOffer" style="display:none;">
			<meta itemprop="offerCount" content="<?=is_array($arResult['OFFERS']) ? count($arResult['OFFERS']) : 0?>" />
...
<? $analogIds = $arResult['PROPERTIES']['ATT_ANALOGI']['VALUE'];
    if(!empty($analogIds)):?>


            <?
            $GLOBALS['arrFilterAnalog'] = array('ID' => $analogIds);
	...
	array(
		...
		"COMPONENT_TEMPLATE" => "catalog_analog",
		"SHOW_RATING" => "Y"
	),
	false
);?>
...
<div class="element_tabs">
    <div tab_id="#InfoTab" class="element_tab active no_link">Описание</div>
    <div tab_id="#CharacteristicsTab" class="element_tab no_link">Характеристики</div>
    <div tab_id="#DeliveryTab" class="element_tab no_link">Доставка</div>
	<?if ($showBlockDocs) {?>
        <a tab_id="#element_docs" class="element_tab no_link">Документы</a>
	<?}?>
    <a href="#ReviewsTab" class="element_tab">Отзывы</a>
    <a href="#AskTab" class="element_tab">Задать вопрос</a>
</div>
<style>
    .element_tab-content{
        display: none;
...
<script>
    $('.no_link').on('click',function (e) {
        $id = $(e.target).attr('tab_id');
        $('.element_tab-content').hide();
        $($id).show();
        CheckActiveTab($(e.target));
    });
    function CheckActiveTab(el){
        $('.no_link').removeClass('active');
        $(el).addClass('active');
    }
...
<?$this->SetViewTarget('PRODUCT_DETAIL_TEXT_INFO');?>
	<?if($arResult["DETAIL_TEXT"]):
		...
		<div class="row element_tab-content" id="InfoTab" style="display: block"> <?//Таб Описание?>


            <?if($arResult['PROPERTIES']['ATT_TOP_TEXT']['VALUE']):?>
            <div class="col-md-12">
                <div class="before-text">
                    <?=$arResult['PROPERTIES']['ATT_BEFORE_IMAGE']['~VALUE']['TEXT']?>
                </div>
                <div class="detail-block-text">
                    <?if($arResult['PHOTO_TOP']):?>
                    <a href="<?=$arResult['PHOTO_TOP']['BIG']['src'];?>"  data-fancybox-group="item_slider_two"  class="block-image popup_link fancy">
                        <img rel="gallery" alt="<?=$arResult['NAME']?>" src="<?=$arResult['PHOTO_TOP']['THUMB']['src'];?>" class="left-block-image fancy">
                    </a>

                    <?endif;?>
                    <div class="right_text">
                        <?=$arResult['PROPERTIES']['ATT_TOP_TEXT']['~VALUE']['TEXT']?>
                    </div>
			...
			<div class="col-md-<?=$class;?>">
				<h2><?=($arParams["TAB_DESCR_NAME"] ? $arParams["TAB_DESCR_NAME"] : GetMessage("DESCRIPTION_TAB"));?></h2>
				<?=$arResult["DETAIL_TEXT"];?>
			</div>



                <?if($arResult['GALLERY_BOTTOM']):?>
                    <div class="col-md-12">
                        <div class="top-line-gallery">
                            <h2 class="slider-gallery__title">Наша фотогалерея</h2>
                        ...
                        <div class="slider-gallery">
                            <?foreach($arResult['GALLERY_BOTTOM']['THUMB'] as $key =>  $item_image):?>
                                <?/*print_r($item_image);*/?>
                                <a href="<?=$arResult['GALLERY_BOTTOM']['BIG'][$key]['src']?>"  data-fancybox-group="item_slider_two"  class="popup_link fancy">
                                    <img rel="gallery" alt="<?=$arResult['NAME']?>" src="<?=$item_image['src']?>" class=" fancy">
                                </a>
...
<div class="element_tab-content col-md-<?=$class;?>" id="DeliveryTab">
    <h2>Доставка</h2>
    <?$APPLICATION->IncludeComponent(
    "bitrix:main.include",
    "",
...
<?if($showBlockDocs):?>
    ...
    //форматы файлов, которые нужно открывать в попапе
    $arPopupFormat = ['jpg','jpeg'];

    if($arFiles):?>
        <div class="element_tab-content" id="element_docs">
            <hr>
            <h2><?=($arParams["BLOCK_DOCS_NAME"] ? $arParams["BLOCK_DOCS_NAME"] : GetMessage("DOCUMENTS_TITLE"))?></h2>
            <div class="files_block">
                <div class="row flexbox">
                    <?foreach($arFiles as $arItem):?>
                        <div class="col-md-3 col-sm-6">
                            <?$arFile=CNext::GetFileInfo($arItem);?>
                            <div class="file_type clearfix <?=$arFile["TYPE"];?> <?=in_array($arFile["TYPE"],$arPopupFormat) ? 'popup-file' : ''?>">
                                <i class="icon"></i>
                                <div class="description">
                                    <a target="_blank" href="<?=$arFile["SRC"];?>" class="dark_link <?=in_array($arFile["TYPE"],$arPopupFormat) ? 'image-source-link' : ''?>"><?=$arFile["DESCRIPTION"];?></a>
                                    <span class="size">
                ...
                callbacks: {
                    open: function() {
                        if (document.documentElement.clientWidth > 768) {
                            waitImg(this.currItem.img[0]).then(function (img) {
                                magnify(img, 3);
                            })
                        }
...
<?$this->SetViewTarget('PRODUCT_PROPS_INFO');?>
	...
	<?if($showProps && ($iCountProps > $arParams['VISIBLE_PROP_COUNT'])):
		$class = 12;?>
		<div class="row char_inner_wrapper element_tab-content" id="CharacteristicsTab"><?//Таб Характеристики?>
			<div class="col-md-<?=$class;?>">
				<h2><?=($arParams["TAB_CHAR_NAME"] ? $arParams["TAB_CHAR_NAME"] : GetMessage("PROPERTIES_TAB"));?></h2>
			...
			<?else:?>
				<div class="gallery-block">
					<div class="gallery-wrapper">
						<div class="inner">
							<?if(is_array($arResult['ADDITIONAL_GALLERY']) && count($arResult['ADDITIONAL_GALLERY']) > 1 || ($arResult['OFFERS'] && 'TYPE_1' === $arParams['TYPE_SKU'])):?>
		...
		<?if($arResult['VIDEO']):?>
			<div class="wraps hidden_print">
				...
				<h4>
					...
					<?if(is_array($arResult['VIDEO']) && count($arResult['VIDEO']) > 1):?>
						<span class="count empty">&nbsp;(<?=is_array($arResult['VIDEO']) ? count($arResult['VIDEO']) : 0?>)</span>
					<?endif;?>
				</h4>
								...
								<?if($arParams["SHOW_KIT_PARTS_PRICES"] == "Y"):?>
									<div class="cost prices clearfix">
										...
										<?if(isset($arSetItem['PRICE_MATRIX']) && $arSetItem['PRICE_MATRIX']):?>
											<?
											// USE_PRICE_COUNT
											if($arSetItem['ITEM_PRICE_MODE'] == 'Q' && is_array($arSetItem['PRICE_MATRIX']['ROWS']) && count($arSetItem['PRICE_MATRIX']['ROWS']) > 1){
