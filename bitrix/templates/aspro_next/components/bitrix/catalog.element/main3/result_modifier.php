...
use Bitrix\Iblock;
/** Переадресация, в случае несовпадения кода товара и ссылки  */
...
if($idElement)
{
	...
	{
		$codeElement = $ar_res['CODE'];
	}
}



if (strcmp($lastElement, $codeElement) !== 0)
{
	// Если регистр не совпадает, делаем редирект 301
    $correctUrl = str_replace($lastElement, $codeElement, $currentUrl);
...
if($arResult['PROPERTIES']['ATT_GALLERY']['VALUE']){
	foreach($arResult['PROPERTIES']['ATT_GALLERY']['VALUE'] as $galleryItem){
		$arResult['GALLERY_BOTTOM']['BIG'][] = CFile::ResizeImageGet($galleryItem, array("width" => 1200, "height" => 1200), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
	}
}

if($arResult['PROPERTIES']['ATT_GALLERY_MIN']['VALUE']){
	foreach($arResult['PROPERTIES']['ATT_GALLERY_MIN']['VALUE'] as $galleryItemMini){
		$arResult['GALLERY_BOTTOM']['THUMB'][] = CFile::ResizeImageGet($galleryItemMini, array("width" => 320, "height" => 500), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
	}
}

if($arResult['PROPERTIES']['ATT_TOP_BG']['VALUE']){
	$arResult['PHOTO_TOP']['BIG'] =  CFile::ResizeImageGet($arResult['PROPERTIES']['ATT_TOP_BG']['VALUE'], array("width" => 1200, "height" => 1200), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());

}


if($arResult['PROPERTIES']['ATT_TOP_BG_MINI']['VALUE']){
	$arResult['PHOTO_TOP']['THUMB'] = CFile::ResizeImageGet($arResult['PROPERTIES']['ATT_TOP_BG_MINI']['VALUE'], array("width" => 320, "height" => 500), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
}
?>
