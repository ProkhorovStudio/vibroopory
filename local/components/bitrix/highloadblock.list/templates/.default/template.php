...
<?
/*function getCountByMonth($highloadBlockId, $dateFieldName = 'UF_DATE', $filter = array(), $limit = 12)
{
    ...
    try {
        ...
        for ($i = 0; $i < $limit; $i++) {
            ...
            $monthEnd->add('1 month -1 second');

            // Формируем фильтр для месяца
            $monthFilter = $filter;
            $monthFilter['>=' . $dateFieldName] = $monthStart;
            $monthFilter['<=' . $dateFieldName] = $monthEnd;

            // Получаем количество записей
            $count = $entityClass::getCount($monthFilter);

            // Формируем результат
            $monthKey = $monthStart->format('Y-m');
            $result[$monthKey] = array(
                'count' => $count,
        ...
        AddMessage2Log('Ошибка при подсчете записей по месяцам: ' . $e->getMessage(), 'highload');
        return array();
    }
...
$result = getCountByMonth(5, $dateFieldName = 'UF_IP_DATA_VIZITA', $filter = array(), $limit = 36);
