...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	protected function getJsDataResult()
	{
		...
		$result['TOTAL'] = [
            ...
            'REAL_BASE_PRICE' => $arResult['REAL_BASE_PRICE'],
            'REAL_BASE_PRICE_FORMATED' => $arResult['REAL_BASE_PRICE_FORMATED'],
            'DISCOUNTS' => $arResult['DISCOUNTS']

		];

        if (empty($result['TOTAL']['DISCOUNTS']) && !empty($result['TOTAL']['REAL_BASE_PRICE']))
        {
            $totalDiscount = 0;
            ...
            foreach ($basket as $basketItem)
            {
                ...
                $discount = ($basePrice - $price) * $basketItem->getQuantity();
                if ($discount > 0)
                {
            ...
            if ($totalDiscount > 0)
            {
                $result['TOTAL']['DISCOUNTS'] = array(array(
                    "NAME" => "Цена для юрлиц",
                    "SUM" => $totalDiscount,
                    "SUM_FORMATED" => SaleFormatCurrency($totalDiscount, $this->order->getCurrency()),
                ));
            }
        }
