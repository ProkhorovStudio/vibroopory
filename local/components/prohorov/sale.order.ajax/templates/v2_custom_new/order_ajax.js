...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var deliveryBlockCustom = document.querySelector('#bx-soa-delivery')
			...
			while (group = groupIterator())
			{
				...
				while (property = propsIterator())
				{
					...
					if (property.getName() === 'Адрес доставки') { //Если свойство совпадает с названием поля адреса в вашей системе
						this.getPropertyRowNode(property, deliveryItemsContainer, false); //вставляем свойство в подготовленный контейнер
						deliveryBlockCustom.querySelector('.bx-soa-section-content').appendChild(deliveryItemsContainer); //контейнер вместе со свойством в нём добавляем в конце блока с описанием (deliveryInfoContainer)
					}

					if (property.getName() === 'Адрес ПВЗ') { //Если свойство совпадает с названием поля адреса в вашей системе
						this.getPropertyRowNode(property, deliveryItemsContainer, false); //вставляем свойство в подготовленный контейнер
						deliveryBlockCustom.querySelector('.bx-soa-section-content').appendChild(deliveryItemsContainer); //контейнер вместе со свойством в нём добавляем в конце блока с описанием (deliveryInfoContainer)
					}

					if (property.getName() === 'Включить стоимость доставки в счет') { //Если свойство совпадает с названием поля адреса в вашей системе
						this.getPropertyRowNode(property, deliveryItemsContainer, false); //вставляем свойство в подготовленный контейнер
						deliveryBlockCustom.querySelector('.bx-soa-section-content').appendChild(deliveryItemsContainer); //контейнер вместе со свойством в нём добавляем в конце блока с описанием (deliveryInfoContainer)
					}
				}
			...
			while (group = groupIterator())
			{
				...
				while (property = propsIterator())
				{
					if (
						...
						|| property.getName() === 'Адрес ПВЗ' //где property.getName() приравниваем к названию поля адреса в вашей системе
						|| property.getName() === 'Включить стоимость доставки в счет' //где property.getName() приравниваем к названию поля адреса в вашей системе
					)
						continue;
		...
		editTotalBlock: function()
		{
			...
			// ДОБАВЛЯЕМ: Вывод скидки из DISCOUNTS (НДС 22%)
			// Важно: используем актуальные данные из this.result.TOTAL
			if (total.DISCOUNTS && BX.type.isArray(total.DISCOUNTS) && total.DISCOUNTS.length > 0)
			{
				for (i = 0; i < total.DISCOUNTS.length; i++)
				{
					var discount = total.DISCOUNTS[i];
					if (discount.NAME && discount.SUM_FORMATED)
					{
						this.totalInfoBlockNode.appendChild(
							this.createTotalUnit('НДС 22%:', discount.SUM_FORMATED, {highlighted: true})
						);
					}
				}
			}
